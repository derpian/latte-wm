...
frameTop (Client * c)
{
    ...
    {
		TRACE ("entering frameTop-end1");
        return c->screen_info->title[TITLE_3][ACTIVE].height;
    }
    TRACE ("entering frameTop-end");
    return 0;
}
...
surface = cairo_xlib_surface_create(title_pm->screen_info->display_info->dpy, title_pm->pixmap, DefaultVisual(title_pm->screen_info->display_info->dpy, 0), title_pm->width, title_pm->height);
...
//            gdk_gc_set_values (gc, &values, GDK_GC_FOREGROUND);
			  gdk_cairo_set_source_color (cr, &screen_info->title_shadow_colors[state].col);
            if (screen_info->params->title_shadow[state] == TITLE_SHADOW_UNDER)
            {
                //gdk_draw_layout (gpixmap, gc, title_x + 1, title_y + 1, layout);
				cairo_move_to (cr, title_x + 1, title_y + 1);
				pango_cairo_show_layout (cr, layout);
            }
            else
            {
                //gdk_draw_layout (gpixmap, gc, title_x - 1, title_y, layout);
                cairo_move_to (cr, title_x - 1, title_y); pango_cairo_show_layout (cr, layout);
                //gdk_draw_layout (gpixmap, gc, title_x, title_y - 1, layout);
                cairo_move_to (cr, title_x, title_y - 1); pango_cairo_show_layout (cr, layout);
                //gdk_draw_layout (gpixmap, gc, title_x + 1, title_y, layout);
                cairo_move_to (cr, title_x + 1, title_y); pango_cairo_show_layout (cr, layout);
                //gdk_draw_layout (gpixmap, gc, title_x, title_y + 1, layout);
                cairo_move_to (cr, title_x, title_y + 1); pango_cairo_show_layout (cr, layout);
...
/*        gdk_gc_set_values (gc, &values, GDK_GC_FOREGROUND);
        ...
        gdk_cairo_set_source_color (cr, &screen_info->title_colors[state].col);

...
surface = cairo_xlib_surface_create(pm->screen_info->display_info->dpy, pm->pixmap, DefaultVisual(pm->screen_info->display_info->dpy, 0), pm->width, pm->height);
...
/* --- */
surface = cairo_xlib_surface_create(pm->screen_info->display_info->dpy, pm->mask, DefaultVisual(pm->screen_info->display_info->dpy, 0), pm->width, pm->height);
    cr = cairo_create(surface);
//	  cr = gdk_cairo_create (dest_bitmap);
...
//    pixbuf = pixbuf_create_from_xpm (&h, color_sym);
    const gchar *buf;
    gint n_col, cpp, items;

    buf = file_buffer (op_header, &h);
    if (!buf)
    {
        g_warning ("Cannot read Pixmap header");
        //return;
    }
    items = sscanf (buf, "%d %d %d %d", &iw, &ih, &n_col, &cpp);
...
ErrorStatus = XpmReadFileToPixmap (myScreenGetXDisplay (screen_info), screen_info->xroot,
                       filename, &pm->pixmap, &pm->mask,
		       &attributes);

//g_warning("XPM: %s, %i, %i ", filename, iw, ih);
g_free (filename);
#ifdef HAVE_RENDER
//g_warning("XPM-err-havrender: %i, %i, %i", ErrorStatus, iw, ih);
    xfwmPixmapRefreshPict (pm);
#endif
//g_warning("XPM-err: %i, %i, %i", ErrorStatus, iw, ih);

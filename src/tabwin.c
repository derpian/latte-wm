...
gtk_widget_get_allocation(GTK_WIDGET(tabwin_widget), allocation); 
    gdouble width = allocation->width;
    gdouble height = allocation->height;
g_free (allocation);

    screen = gtk_widget_get_screen (GTK_WIDGET(tabwin_widget));
    cr = gdk_cairo_create (gtk_widget_get_window(GTK_WIDGET(tabwin_widget)));

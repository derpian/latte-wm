...
print_version (void)
{
    g_print ("\tThis is latte-wm version %s (revision %s) for latte desktop\n",
                    VERSION, REVISION);
    g_print ("\thttp:\/\/latte-desktop.org\/\n\n");
...
static int
initialize (gint compositor_mode, gboolean replace_wm, gchar *theme)
{
    gint i, nscreens;

    TRACE ("entering initialize");

    DBG ("latte-wm starting, using GTK+-%d.%d.%d", gtk_major_version,
    ...
    for(i = 0; i < nscreens; i++)
    {
        ...
        if (!initSettings (screen_info, theme))
        {
            return -2;
        }
        ...
        setUTF8StringHint (main_display_info, screen_info->xfwm4_win, NET_WM_NAME, "Latte");
...
main (int argc, char **argv)
{
    ...
    {
#ifdef HAVE_DAEMON
        { "daemon", '\0', 0, G_OPTION_ARG_NONE, &daemon_mode, ("Fork to the background"), NULL },
#else
        { "daemon", '\0', 0, G_OPTION_ARG_NONE, &daemon_mode, ("Fork to the background (not supported)"), NULL },
#endif
#ifdef HAVE_COMPOSITOR
        { "compositor", '\0', 0, G_OPTION_ARG_CALLBACK, compositor_callback, ("Set the compositor mode"), "on|off|auto" },
#else
        { "compositor", '\0', 0, G_OPTION_ARG_STRING, &compositor_foo, ("Set the compositor mode (not supported)"), "on|off|auto" },
#endif
        { "replace", '\0', 0, G_OPTION_ARG_NONE, &replace_wm, ("Replace the existing window manager"), NULL },
        { "version", 'V', 0, G_OPTION_ARG_NONE, &version, ("Print version information and exit"), NULL },
        { "theme", '\0', 0, G_OPTION_ARG_STRING, &theme_foo, ("Set theme"), NULL },
        { NULL }
    };
    ...
    status = initialize (compositor, replace_wm, theme_foo);
    /*

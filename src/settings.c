...
/*        }
        else
        {
            ...
            if (G_VALUE_TYPE(rc[id].value) == G_TYPE_STRING)
                g_message (("%s: Cannot allocate color %s\n"), g_get_prgname (), g_value_get_string(rc[id].value));
            else
                g_critical (("%s: Cannot allocate color: GValue for color is not of type STRING"), g_get_prgname ());
        } */
    }
    else
    {
        gdk_beep ();
        if (G_VALUE_TYPE(rc[id].value) == G_TYPE_STRING)
            g_message (("%s: Cannot parse color %s\n"), g_get_prgname (), g_value_get_string(rc[id].value));
        else
            g_critical (("%s: Cannot parse color: GValue for color is not of type STRING"), g_get_prgname ());
    }
}
...
static void
loadTheme (ScreenInfo *screen_info, Settings *rc, gchar *t)
{
...
if (t != NULL) {
	//g_warning("using theme %s\n", t);
    theme = g_build_filename (DATADIR, "themes", t, "xfwm4", NULL);
} else {
...
gboolean
loadSettings (ScreenInfo *screen_info, gchar *theme)
{
    const gchar *value;
    Settings rc[] = {
    ...
    loadTheme (screen_info, rc, theme);
...
static gboolean
reloadScreenSettings (ScreenInfo *screen_info, int mask, gchar *theme)
{
    g_return_val_if_fail (screen_info, FALSE);

    unloadSettings (screen_info);
    if (!loadSettings (screen_info, theme))
    {
        return FALSE;
    }
    if (mask)
...
gboolean
reloadSettings (DisplayInfo *display_info, int mask, gchar *theme)
{
    GSList *screens;
    ...
    for (screens = display_info->screens; screens; screens = g_slist_next (screens))
    {
        ...
        if (!reloadScreenSettings (screen_info, mask, theme))
        {
             return FALSE;
        }
    }
...
gboolean
initSettings (ScreenInfo *screen_info, gchar *theme)
{
    GdkKeymap *keymap;
    DisplayInfo *display_info;
    char **names;
